...
async function stepSaveToDb(data: TranscriptResult) {
  ...
  // Build set object with only defined values to avoid "No values to set" error
  const transcriptUpdateSet: Record<string, unknown> = {};
  if (data.transcript !== undefined) {
    transcriptUpdateSet.transcript = data.transcript;
  }
  if (data.viewCount !== undefined) {
    transcriptUpdateSet.viewCount = data.viewCount;
  }
  if (data.likes !== undefined) {
    transcriptUpdateSet.likeCount = data.likes;
  }

  // Use onConflictDoNothing if no values to update, otherwise use onConflictDoUpdate
  if (Object.keys(transcriptUpdateSet).length === 0) {
    await db
      ...
      .onConflictDoNothing({ target: scrapTranscriptV1.videoId });
  } else {
    await db
      .insert(scrapTranscriptV1)
      .values({
