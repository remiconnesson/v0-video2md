...
export async function fetchYoutubeTranscriptFallback(
  ...
  console.log(
    "[youtubei.js] Attempting fallback to ANDROID client via /player endpoint...",
  );
  ...
  if (!videoDetails) {
    throw new Error(
      "[youtubei.js] Fallback failed: No videoDetails in player response",
    );
  }

  console.log(`[youtubei.js] Fallback metadata found: ${videoDetails.title}`);
  ...
  if (captionTracks && captionTracks.length > 0) {
    ...
    if (preferredTrack) {
      ...
      console.log(
        `[youtubei.js] Fallback: Fetching subtitles from ${preferredTrack.baseUrl}`,
      );
      ...
      if (subtitleResponse.ok) {
        ...
        console.log(
          `[youtubei.js] Fallback: Parsed ${segments.length} subtitle segments`,
        );
      } else {
        console.warn(
          `[youtubei.js] Fallback: Failed to fetch subtitles: ${subtitleResponse.status}`,
        );
      }
    }
  }
  ...
  if (segments.length === 0) {
    throw new Error(
      `[youtubei.js] Fallback failed: No transcripts could be extracted`,
    );
  }
  ...
  return {
    ...
    date: "", // Date is hard to get from Android response without microformats
