...
export async function fetchYoutubeTranscriptFallback(
  ...
  originalTlsReject: string | undefined,
): Promise<TranscriptResult> {
  const clients = ["ANDROID", "IOS"];
  let lastError: Error | null = null;

  for (const client of clients) {
    try {
      console.log(
        `[youtubei.js] Attempting fallback to ${client} client via /player endpoint...`,
      );
      ...
      if (!videoDetails) {
        const keys = playerJson ? Object.keys(playerJson).join(", ") : "null";
        console.warn(
          `[youtubei.js] Fallback (${client}) failed: No videoDetails. Available keys: ${keys}`,
        );
        throw new Error(`No videoDetails in ${client} response`);
      }

      console.log(
        `[youtubei.js] Fallback (${client}) metadata found: ${videoDetails.title}`,
      );
      ...
      if (captionTracks && captionTracks.length > 0) {
        ...
        if (preferredTrack) {
          ...
          console.log(
            `[youtubei.js] Fallback (${client}): Fetching subtitles from ${preferredTrack.baseUrl}`,
          );

          // Force VTT format by replacing fmt param or appending it
          let subtitleUrl = preferredTrack.baseUrl;
          if (subtitleUrl.includes("fmt=")) {
            subtitleUrl = subtitleUrl.replace(/fmt=[^&]+/, "fmt=vtt");
          } else {
            subtitleUrl += "&fmt=vtt";
          }
          ...
          if (subtitleResponse.ok) {
            ...
            console.log(
              `[youtubei.js] Fallback (${client}): Parsed ${segments.length} subtitle segments`,
            );
          } else {
            console.warn(
              `[youtubei.js] Fallback (${client}): Failed to fetch subtitles: ${subtitleResponse.status}`,
            );
          }
        }
      }
      ...
      return {
        ...
        date: "", // Date is hard to get from Android/iOS response without microformats
        channelId: videoDetails.channelId ?? "",
        channelName: videoDetails.author ?? "Unknown Channel",
        description: videoDetails.shortDescription ?? "",
        numberOfSubscribers: 0,
        viewCount: Number(videoDetails.viewCount ?? 0),
        likes: 0,
        duration: formatDurationFromSeconds(
          Number(videoDetails.lengthSeconds ?? 0),
        ),
        isAutoGenerated,
      ...
      console.warn(
        `[youtubei.js] Fallback (${client}) attempt failed: ${e.message}`,
      );
      lastError = e;
  ...
  throw new Error(
    `[youtubei.js] All fallback attempts failed. Last error: ${lastError?.message}`,
  );
}
...
export async function fetchYoutubeTranscriptFromYoutubei(
  ...
  try {
    ...
    if (!basic_info.title) {
      throw new Error("Metadata title is missing (parser error?)");
    }
    ...
    console.warn(
      `[youtubei.js] Primary fetch failed for ${videoId}. Trying fallback...`,
      error,
    );
    return fetchYoutubeTranscriptFallback(
      yt,
      videoId,
      proxyAgent,
      disableTlsVerify,
