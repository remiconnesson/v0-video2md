...
// ============================================================================
// Slide Markdown Analysis Tables
// ============================================================================

/**
 * Status of AI analysis run for slides
 */
export const slideAnalysisRuns = pgTable(
  "slide_analysis_runs",
  {
    id: serial("id").primaryKey(),
    videoId: videoIdColumn(),
  ...
  (table) => [
    index("slide_analysis_runs_video_idx").on(table.videoId),
    unique("slide_analysis_runs_video_unique").on(table.videoId),
  ],
);

export type SlideAnalysisRun = typeof slideAnalysisRuns.$inferSelect;
export type NewSlideAnalysisRun = typeof slideAnalysisRuns.$inferInsert;

/**
 * AI-generated markdown for individual slides
 */
export const slideMarkdown = pgTable(
  "slide_markdown",
  {
    id: serial("id").primaryKey(),
    videoId: videoIdColumn(),
    slideNumber: integer("slide_number").notNull(),
    framePosition: framePositionEnum("frame_position").notNull(), // which frame was analyzed
    markdown: text("markdown"), // AI-generated markdown content
    createdAt: timestamp("created_at").defaultNow().notNull(),
  },
  (table) => [
    index("slide_markdown_video_idx").on(table.videoId),
    unique("slide_markdown_video_slide_frame").on(
      table.videoId,
      table.slideNumber,
      table.framePosition,
    ),
...
export type SlideMarkdown = typeof slideMarkdown.$inferSelect;
export type NewSlideMarkdown = typeof slideMarkdown.$inferInsert;

/**
 * Feedback on AI-generated slide markdown (thumbs up/down + comments)
 */
export const slideMarkdownFeedbackEnum = pgEnum("slide_markdown_feedback", [
  "positive",
  "negative",
]);

export type SlideMarkdownFeedbackType =
  (typeof slideMarkdownFeedbackEnum.enumValues)[number];

export const slideMarkdownFeedback = pgTable(
  "slide_markdown_feedback",
  {
    id: serial("id").primaryKey(),
    videoId: videoIdColumn(),
    slideNumber: integer("slide_number").notNull(),
    framePosition: framePositionEnum("frame_position").notNull(),
    feedback: slideMarkdownFeedbackEnum("feedback"), // positive or negative
    comment: text("comment"), // optional user comment
    createdAt: timestamp("created_at").defaultNow().notNull(),
  },
  (table) => [
    index("slide_markdown_feedback_video_idx").on(table.videoId),
    unique("slide_markdown_feedback_video_slide_frame").on(
      table.videoId,
      table.slideNumber,
      table.framePosition,
    ),
...
export type SlideMarkdownFeedback = typeof slideMarkdownFeedback.$inferSelect;
export type NewSlideMarkdownFeedback =
  typeof slideMarkdownFeedback.$inferInsert;
